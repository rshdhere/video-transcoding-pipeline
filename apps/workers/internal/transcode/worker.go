...
func (w *Worker) processNext(ctx context.Context) error {
	...
	if err != nil {
		if err == store.ErrJobNotQueued {
			if deleteErr := w.queue.Delete(ctx, message.ReceiptHandle); deleteErr != nil {
				return deleteErr
			}
			return nil
		}
	...
	if record.VideoID == nil || *record.VideoID == "" {
		return w.failJob(ctx, record.ID, message.ReceiptHandle, "transcoding job is missing videoId")
	}
...
func (w *Worker) transcodeVideo(ctx context.Context, videoID string) error {
	...
	return w.store.UpdateVideoStatus(ctx, videoID, "completed")
}
